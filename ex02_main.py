...
def test(model, testloader, diffusor, device, args):
    ...
    - also generate 8 samples and store them under args.save_dir/samples
    """
    model.eval()
    total_loss = 0.0
    n_batches = 0
    ...
    with torch.no_grad():
        for images, labels in tqdm(testloader, desc="validation"):
            ...
            # Compute how well the model predicts the noise added to images at timestep t.
    ...
    # Generate a small grid of images for monitoring
