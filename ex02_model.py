...
class Unet(nn.Module):
    def __init__(
        ...
        # Class embedder for classifier-free guidance
        self.class_free_guidance = class_free_guidance
        self.p_uncond = default(p_uncond, 0.1)  # Probability of unconditional training
        self.num_classes = num_classes
        
        if self.class_free_guidance:
            assert num_classes is not None, "num_classes must be provided for classifier-free guidance"
            classes_dim = dim * 4
            # +1 for the null token (unconditional)
            self.class_emb = nn.Embedding(num_classes + 1, classes_dim)
            self.null_class = num_classes  # Use last index as null token
        ...
        # Adapt all blocks to accommodate class embeddings
    ...
    def forward(self, x, time, classes=None):
        ...
        Args:
            ...
            classes: Class labels [B] or None for unconditional generation
        """
        x = self.init_conv(x)
        r = x.clone()
        ...
        # Class conditioning with classifier-free guidance support
        c = None
        if self.class_free_guidance:
            if classes is not None:
                # During training: randomly replace some class labels with null token
                if self.training:
                    # Create mask for unconditional samples
                    uncond_mask = torch.rand(classes.shape[0], device=classes.device) < self.p_uncond
                    # Replace with null token where mask is True
                    classes = torch.where(uncond_mask, 
                                        torch.tensor(self.null_class, device=classes.device), 
                                        classes)
                
                # Get class embeddings and process through MLP
                c = self.class_emb(classes)
                c = self.classes_mlp(c)
            else:
                # If no classes provided but model is conditional, use null token for all
                batch_size = x.shape[0]
                null_classes = torch.full((batch_size,), self.null_class, device=x.device, dtype=torch.long)
                c = self.class_emb(null_classes)
                c = self.classes_mlp(c)
